...
//   output logic                             intr_cmd_complete_o
  ...
  localparam int unsigned MaxFifoDepthW = FifoDepth;
  ...
  localparam int unsigned RoundTripCycles = 2 + 1;
  ...
  //hw2reg.status.targetidle.d = target_idle;
  ...
  assign hw2reg.controller_events.nack.d  = 1'b0;
  assign hw2reg.controller_events.nack.de = event_nak;
  assign hw2reg.controller_events.unhandled_nack_timeout.d  = 1'b0;
  assign hw2reg.controller_events.unhandled_nack_timeout.de = event_unhandled_nak_timeout;
  assign hw2reg.controller_events.bus_timeout.d  = 1'b0;
  assign hw2reg.controller_events.bus_timeout.de = 1'b0 && !host_idle;
  assign hw2reg.controller_events.arbitration_lost.d  = 1'b0;
  ...
  // hw2reg.target_events.tx_pending.de = event_read_cmd_received &&
  ...
  assign scl_o = scl_out_fsm;
  assign sda_o = sda_out_fsm;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      scl_out_fsm <= scl_out_controller_fsm;
      sda_out_fsm <= sda_out_controller_fsm;
    end
  end
...
// Reg2hw Assigns
  ...
  assign t_r                          = 10'(reg2hw.timing1.t_r.q);
  assign t_f                          = 9'(reg2hw.timing1.t_f.q);
  assign tsu_sta                      = reg2hw.timing2.tsu_sta.q;
  assign thd_sta                      = reg2hw.timing2.thd_sta.q;
  assign tsu_dat                      = 9'(reg2hw.timing3.tsu_dat.q);
  ...
  assign stretch_timeout_enable       = reg2hw.timeout_ctrl.en.q &&
                                        (reg2hw.timeout_ctrl.mode.q == 1'b0);
  assign bus_timeout_enable           = reg2hw.timeout_ctrl.en.q &&
                                        (reg2hw.timeout_ctrl.mode.q == 1'b1);
  ...
  assign scl_sync = scl_i;
  assign sda_sync = sda_i;
  ...
  assign sda_fsm = sda_out_controller_fsm;
  assign scl_fsm = scl_out_controller_fsm;
  ...
  assign target_arbitration_lost = sda_released_but_low;
  assign multi_controller_mon_en = reg2hw.ctrl.multi_controller_monitor_en.q;
  ...
  //   .multi_controller_enable_i      (multi_controller_mon_en),
  //   .target_enable_i                (target_enable),
  //   .target_idle_i                  (1),
  //   .thd_dat_i                      (thd_dat),
  //   .t_buf_i                        (t_buf),
  //   .bus_active_timeout_i           (0),
  //   .bus_active_timeout_en_i        (bus_timeout_enable),
  //   .bus_inactive_timeout_i         (0),
  ...
  i2c_controller_fsm #(
    ...
    .bus_free_i                     (1),                          // Open Titan TLUL Bus free
    ...
    .unhandled_unexp_nak_i          (0),
    .unhandled_nak_timeout_i        (0),
    ...
    .stretch_timeout_i              (bus_active_timeout),                     // Max time slave may stretch the clock
    .timeout_enable_i               (stretch_timeout_enable),
    .host_nack_handler_timeout_i    (host_nack_handler_timeout),                     // Timeout threshold for unhandled Host-Mode 'nak'
