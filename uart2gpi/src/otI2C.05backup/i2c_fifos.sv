...
// Target fifo's excluded
`timescale 1ns / 1ps


module i2c_fifos #(
    parameter int unsigned FifoDepth = 8,  // REDUCED from 64!
    parameter int unsigned FifoDepthW = $clog2(FifoDepth + 1)
) (
    input  logic                         clk_i,
    input  logic                         rst_ni,
...
//               RamWidth >= ACQ_FIFO_WIDTH)
    ...
    // ogic rx_ram_req, rx_ram_gnt, rx_ram_write, rx_ram_rvalid;
