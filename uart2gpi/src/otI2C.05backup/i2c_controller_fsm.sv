...
#(
  ...
  parameter int RX_FIFO_WIDTH = 8,
  parameter int FifoDepthWidth = $clog2(FifoDepth+1)
) (
  input        clk_i,  // clock
  ...
  input [12:0] thigh_i,    // high period of the SCL in clock units                                          // Reduced from [12:0]
  input [12:0] tlow_i,     // low period of the SCL in clock units                                           // Reduced from [12:0]
  input [12:0] t_r_i,      // rise time of both SDA and SCL in clock units                                   // Reduced from [12:0]
  input [12:0] t_f_i,      // fall time of both SDA and SCL in clock units                                   // Reduced from [12:0]
  input [12:0] thd_sta_i,  // hold time for (repeated) START in clock units                                  // Reduced from [12:0]
  input [12:0] tsu_sta_i,  // setup time for repeated START in clock units                                   // Reduced from [12:0]
  input [12:0] tsu_sto_i,  // setup time for STOP in clock units                                             // Reduced from [12:0]
  input [12:0] thd_dat_i,  // data hold time in clock units                                                  // Reduced from [12:0]
  input        sda_interference_i, // High when SCL is high and SDA doesn't match while transmitting
  input [29:0] stretch_timeout_i,  // max time target connected to this host may stretch the clock           // Reduced from [29:0]
  input        timeout_enable_i,   // assert if target stretches clock past max
  input [9:0] host_nack_handler_timeout_i, // Timeout threshold for unhandled Host-Mode 'nak' irq.          // Reduced from [30:0]
  ...
  logic [9:0] stretch_idle_cnt; // counter for clock being stretched by target
                                 // or clock idle by host.
  logic [9:0] unhandled_nak_cnt; // In Host-mode, count cycles where the FSM is halted awaiting
  ...
  logic [9:0] stretch_cnt_threshold;
